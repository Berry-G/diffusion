...
- **연결되지 않은 노드는 기록에서 빼야 한다.** 이 워크플로우에는 아무 데도 안 붙은
...
- **최종 접속 주소는 전용 서브도메인 하나**이고(실제 값은 저장소에 두지 않는다 —
  `tools/cert.local.bat` 에 있다), Tailscale 로그인한 기기만 닿을 수 있게
